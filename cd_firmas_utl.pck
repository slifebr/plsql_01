create or replace package cd_firmas_utl is
 ...
 --------------------------------------------------------------------------------
   ...
   */
   ...
   --------------------------------------------------------------------------------
   function endereco_com_numero(cod in cd_firmas.firma%type) return cd_firmas.endereco%type;
   pragma restrict_references(endereco_com_numero,
                              wnds,
                              wnps);                            
   --------------------------------------------------------------------------------
...
create or replace package body cd_firmas_utl is
      ...
      cursor cr is
        select 'S'
         ...
         where f.firma = p_firm
           and f.tipo_firma in ('FUNCIONARIO','COLABORADOR');
 ...
 --------------------------------------------------------------------------------
   ...
   begin
      select endereco||decode(f.numero,null,'',' - '|| f.numero) into v_campo from cd_firmas f where firma = cod;
