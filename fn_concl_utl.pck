...
--|----------------------------------------------------------------------
  ...
  begin
    ...
    for reg in cr_ext loop
      ...
      if fn_concl_utl.retorna_chave_ofx(reg.linha) = '</STMTTRN>' then
        reg_linha_ofx.v_ind_fim := reg.num_linha;
        exit;
      --<FITID>N1011E
      --/ id da transacao
      elsif fn_concl_utl.retorna_chave_ofx(reg.linha) = '<FITID>' then
        v_id_trans := fn_concl_utl.retorna_valor_ofx(reg.linha,
                                                          '<FITID>');
      --/ operacao
      elsif fn_concl_utl.retorna_chave_ofx(reg.linha) = '<TRNTYPE>' then
        reg_linha_ofx.v_trans := substr(fn_concl_utl.retorna_valor_ofx(reg.linha,
                                                                   '<TRNTYPE>'),
                                    1,
                                    1);
        if reg_linha_ofx.v_trans = 'C' then
      ...
      --/ data movto
      elsif fn_concl_utl.retorna_chave_ofx(reg.linha) = '<DTPOSTED>' then
      ...
      --/ valor
      elsif fn_concl_utl.retorna_chave_ofx(reg.linha) = '<TRNAMT>' then
        v_vl_txt        := fn_concl_utl.retorna_valor_ofx(reg.linha,
                                                          '<TRNAMT>');
        reg_linha_ofx.v_valor := to_number(v_vl_txt) * v_sinal;

      --/ documento
      elsif fn_concl_utl.retorna_chave_ofx(reg.linha) = '<CHECKNUM>' then
        reg_linha_ofx.v_doc := fn_concl_utl.retorna_valor_ofx(reg.linha,
                                                          '<CHECKNUM>');
    
      --/ historico
      elsif fn_concl_utl.retorna_chave_ofx(reg.linha) = '<MEMO>' then
  ...
  exception 
    when others then
      raise_application_error(-20200,v_linha);
  end;
  --|----------------------------------------------------------------------
  ...
  begin
    ...
    if retorno is not null then
      retorno := trim(substr(retorno, length(chave) + 1));
  ...
  Begin
    ...
    if fn_concl_utl.retorna_valor_ofx('<BANKID>') != v_banco or
       fn_concl_utl.retorna_valor_ofx('<ACCTID>') != reg_cta.cod_conta then
      raise_application_error(-20100,
      p_emp  ||' - '||
      p_banco ||' - '||
      p_agenc ||' - '||
      p_cta  ||' - '||
                              ...
                              'Banco('||v_banco||')/Conta-[cod_conta]('||reg_cta.cod_interno||')'||', não conferem com extrato importado!');
    end if;
